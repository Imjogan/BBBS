...
const RightPage = () => {
  ...
  useEffect(() => {
    apiServer
      ...
      .then((res) => {
        console.log(res);
        setRight(res);
        setCurrentColor(colorArray.find((obj) => Object.keys(obj)[0] === res.color));
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  return (
    <section className="content-rights">
      <section
        className={`content__header content__header_style_rights-article ${
          Object.values(currentColor)[0]
        }`}
      >
        <div
          className={`content__pseudo-blocks ${Object.values(currentColor)[0]}`}
        ></div>
        <div className="content__wrapper">
          <h1 className="title title_style_rights-article">
            {right.title}
          </h1>
